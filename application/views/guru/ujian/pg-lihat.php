                        ...
                        <?php if (strtotime(date('Y-m-d H:i', time())) > strtotime($ujian->waktu_berakhir)) : ?>
                            <ul class="list-group list-group-media mt-2">
                                ...
                                <?php foreach ($siswa as $s) : ?>
                                    <?php $belum_terjawab = $this->db->get_where('ujian_siswa', ['id_ujian' => $ujian->id_ujian, 'id_siswa' => $s->id_siswa, 'jawaban' => NULL])->result(); ?>
                                    <?php $benar = $this->db->get_where('ujian_siswa', ['id_ujian' => $ujian->id_ujian, 'id_siswa' => $s->id_siswa, 'benar' => 1])->result(); ?>
                                    <?php $salah = $this->db->get_where('ujian_siswa', ['id_ujian' => $ujian->id_ujian, 'id_siswa' => $s->id_siswa, 'benar' => 0])->result(); ?>
                                    <a href="<?= base_url('guru/pg_siswa/') . encrypt_url($s->id_siswa) . '/' . encrypt_url($ujian->id_ujian); ?>">
